...
class Solution {
  ...
  int convertToIndex(int x, int y, int totalCols) { return x * totalCols + y; }
        ...
        if (isCellLand(x, y, grid2)) {
          ...
          for (auto &direction : directions) {
            ...
            if (nextX >= 0 && nextY >= 0 && nextX < totalRows &&
                nextY < totalCols && isCellLand(nextX, nextY, grid2)) {
              uf.unionSets(convertToIndex(x, y, totalCols),
