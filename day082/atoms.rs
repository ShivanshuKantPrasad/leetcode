...
pub fn count_of_atoms(formula: String) -> String {
    ...
    fn parse_atoms(formula: &[u8]) -> HashMap<String, i32> {
        ...
        while i < formula.len() {
            ...
            if formula[i] == b'(' {
                ...
                let end = i;
                i += 1;
                let sub_freq = parse_atoms(&formula[start..end]);
                let mut mult = 0;
                ...
                mult = mult.max(1);
                let key = String::from_utf8(formula[start..end].to_vec()).unwrap();
    ...
    let freq = parse_atoms(formula.as_bytes());
    // println!("{freq:?}");
